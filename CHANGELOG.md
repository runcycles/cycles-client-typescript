...
- **Heartbeat extend drift (P1 liveness).** Per the protocol spec, `extend_by_ms` extends relative to the *current* `expires_at_ms`, and the server caps `extension_count` at `max_extensions` (default 10). Both heartbeats (`withCycles` and `reserveForStream`) beat every `max(ttlMs/2, 1000)` ms and extended by the full `ttlMs` on **every** beat, so expiry drifted outward by `ttl/2` per beat — a process crash left the budget reserved until the drifted expiry (zombie budget lockup) — and the extension budget burned twice as fast as needed, so runs longer than about `max_extensions * ttl/2` lost heartbeat protection mid-flight. Both heartbeats now use **lead-estimate extension**: each beat (every `ttlMs/2`, the 1 s interval floor removed — it could only ever bind for `ttl < 2000`, where beating slower than `ttl/2` guarantees a lapse; spec minimum `ttl_ms` is 1000, so the interval is ≥ 500 ms) estimates the expiry *lead* as `(knownExpiry − initialExpiry) + ttl − (now − anchor)` and skips the extend only when the lead is already ≥ `1.5 * ttl` (steady-state pattern: extend, extend, skip, …). The estimate is clock-skew-free — it only subtracts server timestamps from server timestamps and client-monotonic timestamps from client-monotonic timestamps, never client wall clock vs. server expiry — and self-corrects from the authoritative `expires_at_ms` in each extend response (a server-clamped grant just makes the next beats extend again); a 2xx without a numeric `expires_at_ms` counts optimistically as `+ttl` of lead. A failed extend is retried on the next beat **reusing the same idempotency key**, so a lost response cannot double-extend; a success regenerates the key. Permanent rejections — HTTP 410, `RESERVATION_EXPIRED`, `RESERVATION_FINALIZED`, `MAX_EXTENSIONS_EXCEEDED`, `TENANT_CLOSED` (tenant closure is irreversible), or `NOT_FOUND` (a 404'd reservation never comes back) — now stop the heartbeat for good (previously it retried doomed extends forever); transient failures log a warning and keep retrying. Expiry lead stays within `[ttl/2, 1.5 * ttl]` at steady state with no drift, one extension consumed per `ttl` of runtime, and `ctx.expiresAtMs` is still updated from the authoritative extend response. *(Replaces the alternate-beat scheme shipped earlier on this branch: adversarial review showed alternate-beat put every steady-state attempt at exactly `ttl/2` lead — one failed extend dropped the retry to lead 0 — and its 1 s interval floor guaranteed a lapse for spec-legal `ttl < 2000`.)*
- **Heartbeat seeded from the EFFECTIVE granted TTL, not the requested one.** Tenant policy `max_reservation_ttl_ms` silently **caps** the granted TTL at reserve time (governance default: 1 hour) and the create response carries no effective-TTL field, so a heartbeat seeded from the requested `ttl_ms` scheduled its first beat far too late — a 24 h request capped to 1 h put the first beat at 12 h, 11 h after the reservation expired. The client now captures the HTTP `Date` response header (exposed as `CyclesResponse.serverDateMs`) and both heartbeats compute `effectiveTtl = clamp(expires_at_ms − serverDateMs, 1000, requestedTtl)` at heartbeat start, falling back to the requested TTL when either server timestamp is absent or unparseable. Both terms are server-frame, so the computation stays clock-skew-free (HTTP-date's ~1 s resolution is absorbed by the heartbeat margins). The effective TTL drives everything in the heartbeat: the beat interval (`effectiveTtl/2`), the lead formula's ttl term, the `1.5×` skip threshold, and `extend_by_ms`.
