...
- **Server-authoritative heartbeat scheduling via `remaining_ttl_ms` (spec PR #148, review round 5).** The protocol adds `remaining_ttl_ms` (integer, int64, ≥ 0) to both `ReservationCreateResponse` and `ReservationExtendResponse`: the remaining reservation lifetime in ms at response evaluation, in the same clock snapshot as `expires_at_ms`, present on successful live-reservation responses (absent on dry-run/DENY and on older servers; `cycles-server` is implementing emission in parallel). Both heartbeats parse it (create/extend mappers plus the raw extend body) and, when present, schedule **normatively**: with `rtt` the measured monotonic round-trip of that call (0 when unknown; the per-heartbeat maximum is tracked), `leadFloor = max(0, remaining_ttl_ms − rtt)`, `retryReserve = min(leadFloor/2, max(1 s, 2 × maxObservedRtt))`, and the next beat fires `max(0, leadFloor − retryReserve)` after response receipt — recomputed from *every* successful response carrying the field; expiry differences are never accumulated in this mode, and the heuristic `leadMin` skip check is **bypassed** (the schedule is exact; a heuristic skip could push a beat past the real lease). When the **create** response carries the field, the first beat is derived from it with the same formula instead of the immediate prime — a request capped to a 1 s lease gets its first beat at 500 ms, inside the real lease, and no `max_extensions` slot is wasted priming under a max-lead clamp. Transient failures retry with the **same idempotency key** after `clamp(currentLead/4, 1 s, 30 s)`, where `currentLead` is the last `leadFloor` decayed by monotonic elapsed time. The grants/lead bookkeeping keeps running underneath, so if the field disappears mid-flight (proxy strips it, mixed-version fleet) the v2.3 heuristic takes over seamlessly; servers that never send it get the unchanged fallback behavior.
