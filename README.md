...
[![Recovery conformance](https://img.shields.io/github/actions/workflow/status/runcycles/cycles-client-typescript/ci.yml?branch=main&label=recovery%20conformance)](https://runcycles.io/protocol/sdk-recovery-conformance)
...
new CyclesConfig({
  ...
  retryFlushTimeout: 10_000, // default: 10000 (ms) — flush() bound

  // Commit durability (on-disk journal, survives process restarts)
  journalEnabled: true,      // default: true
  journalDir: undefined,     // default: ~/.runcycles/commit-journal
});
```
...
## Retry Behavior & Commit Durability

A commit records spend that has already happened, so the SDK never lets one exist only in memory. When a commit fails transiently (transport error, 5xx, or 429), the client journals it to disk and schedules background retries with exponential backoff:

- **Durable journal** — every scheduled retry is first written to `journalDir` (default `~/.runcycles/commit-journal`) in a per-identity subdirectory (directories `0700`, files `0600` where supported) keyed by a non-secret PBKDF2 fingerprint of the server plus principal: the configured `tenant` when set (stable across API-key rotation), else the API key. Entries are removed only on a terminal outcome; survivors replay automatically the next time a client lifecycle is created for the same identity. The derivation is byte-compatible with the Python SDK, so same-tenant clients in both languages share an identity directory.
...
- **Success contract (both scheduling modes):** only a complete, schema-valid HTTP 200 create/extend response counts as success. A malformed or non-200 2xx response is ambiguous and is recovered with the same idempotency key; the fallback changes only how the next delay is calculated, not what proves that an operation succeeded.
- **Server-authoritative scheduling (`remaining_ttl_ms`, spec PR #148) — the normative path.** When a create or extend response carries `remaining_ttl_ms` (remaining lifetime at response evaluation, same clock snapshot as `expires_at_ms`; present on successful live-reservation responses), the heartbeat schedules from it exactly, recomputing from every *schema-valid* response alone: `lead_floor = max(0, remaining_ttl_ms − rtt)` with `rtt` that attempt's measured monotonic round-trip (max tracked); `attempt_budget = max(request timeout, 1s, 2×maxRtt)` where the request timeout is the client's *enforced* per-attempt bound (`connectTimeout + readTimeout`, applied to every request via `AbortSignal.timeout`); `safety_margin = max(1s, 2×maxRtt)`; and the next beat fires `max(0, lead_floor − (2×attempt_budget + safety_margin))` after response receipt. Only a schema-valid HTTP 200 `ReservationExtendResponse` counts as an observed success on this path — any other or malformed 2xx is ambiguous and recovered like a transient failure with the same idempotency key. The lead-accounting skip check is bypassed (the schedule is exact), and the first beat is derived from the create response with the create call's own measured rtt instead of firing immediately. A schema-valid success that still yields a zero delay permits exactly one immediate fresh attempt (new key); a second consecutive zero stops the heartbeat and surfaces that the lease is shorter than the retry-safety budget (unreliable timing or an unbounded timeout takes the same path — never a silent downgrade to the fallback). Transient failures (timeout, connection error, 5xx, 429, ambiguous 2xx) recover with the same key inside an unclamped `retry_window = current_lead − attempt_budget − safety_margin`: negative → stop and surface; otherwise non-429 retries after `min(30s, current_lead/4, retry_window)`, and 429 honors `Retry-After` exactly and only when it fits the window. Any other 4xx stops without ever rotating the idempotency key. Everything below is the **fallback** for servers that do not send the field; if the field disappears mid-flight, the fallback takes over seamlessly
- **Measured-grant lead accounting (fallback):** the server extends relative to the *current* `expires_at_ms`, so each beat computes a rigorous *lower bound* on the expiry lead — `leadMin = grantsSum − elapsed`, where grants are differences of successive server-returned `expires_at_ms` values (same server clock frame) and `elapsed` is client-monotonic. No cross-clock arithmetic anywhere, and `leadMin` starts at 0 (the initial grant is never counted — there is no safe same-clock anchor for it), so the bound never overstates the real lead. A beat is skipped only when `leadMin ≥ 1.5 ×` the last *measured* grant; steady state consumes about one of the server's `max_extensions` (default 10) per grant of runtime with no outward expiry drift
- **First beat (fallback): immediate (delay 0).** Tenant policy `max_reservation_ttl_ms` can silently cap the granted TTL below the requested one (governance default: 1 hour), and without `remaining_ttl_ms` *any* bounded first-beat delay can outlive a small capped lease — so the first extend fires right away and primes the lease with a real, measurable grant. The 0 delay applies to the first schedule only: a transiently failed first attempt retries after the full held interval (below), never hot-loops. The HTTP `Date` header plays no role anywhere in the heartbeat (`CyclesResponse.serverDateMs` remains as a general response accessor): per RFC 9110 `Date` is a whole-second best-effort origination time that intermediaries may replace, and in the reference server `expires_at_ms` comes from Redis `TIME` while `Date` comes from the servlet container — never a safe same-clock pair
- **Cadence (fallback, regime split):** a grant may drive the cadence only when it is a real *per-extend* amount. After such a grant the interval re-derives as `clamp(grant/2, 500ms, requestedTtl/2)` — e.g., a 60s TTL granted in full heartbeats every 30s, while a 24h request silently capped to 1h-per-extend tightens to 30min beats automatically. But under a maximum-**lead** clamp (the server holds `expires_at_ms ≈ now + L`), successive `expires_at_ms` differences measure *elapsed time*, not lease — deriving the cadence from them would collapse the interval to the 500ms floor and burn `max_extensions` in seconds. A grant that is non-positive, or both `< 0.9×` the requested ttl and within the two-sided `0.75×`–`1.25×` band of the elapsed time since the last applied extend, therefore *holds* the cadence at `min(requestedTtl/2, 30s)`, logs a once-per-heartbeat `console.warn` (the server appears to clamp lease lead; the extension budget will deplete), and keeps extending every beat. The band is two-sided on purpose: after a `leadMin` skip the next grant arrives across a doubled gap, so a genuine grant-clamped server also shows `grant ≈ elapsed` exactly once — it lands in the hold once, then at the held cadence its grant/elapsed ratio falls to ≈0.5, exits the band, and the cadence re-tightens (an upper bound alone would make that hold self-sustain and decay the lease). A real maximum-lead clamp tracks any gap at ratio ≈1 and stays held. The honest limit: regime detection from `(grant, elapsed)` alone is undecidable in general — any grant in `[0.75×min(ttl/2, 30s), 0.9×ttl)` reproduces the elapsed gap at the held cadence and misclassifies permanently — which is exactly why `remaining_ttl_ms` above is the normative path and this heuristic is best-effort for servers clamping only the per-extend delta
- **Extension amount:** `extend_by_ms` equals the full **requested** `ttl_ms` each time — the server owns clamping; the client never shrinks its ask based on estimates
...
- **Permanent-failure stop:** HTTP 410, `RESERVATION_EXPIRED`, `RESERVATION_FINALIZED`, `MAX_EXTENSIONS_EXCEEDED`, `TENANT_CLOSED`, or `NOT_FOUND` stops the heartbeat permanently — no retry can fix those
