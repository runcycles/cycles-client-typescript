...
- **First beat:** at `min(requestedTtl/2, 30s)` — tenant policy `max_reservation_ttl_ms` can silently cap the granted TTL below the requested one (governance default: 1 hour) with no effective-TTL response field, so the first beat lands early no matter what was granted. The HTTP `Date`-derived grant estimate (`expires_at_ms − Date`), when available, can only *tighten* this delay: per RFC 9110 `Date` is a whole-second best-effort origination time that intermediaries may replace (and in the reference server `expires_at_ms` comes from Redis `TIME` while `Date` comes from the servlet container), so it is a cadence hint only — never a correctness input
- **Cadence:** after each applied grant the interval re-derives from the *measured* grant as `clamp(lastGrant/2, 500ms, requestedTtl/2)` — e.g., a 60s TTL granted in full heartbeats every 30s, while a 24h request silently capped to 1h-per-extend tightens to 30min beats automatically
