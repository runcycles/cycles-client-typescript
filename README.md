...
- **Server-authoritative scheduling (`remaining_ttl_ms`, spec PR #148) — the normative path.** When a create or extend response carries `remaining_ttl_ms` (remaining lifetime at response evaluation, same clock snapshot as `expires_at_ms`; present on successful live-reservation responses), the heartbeat schedules from it exactly: `leadFloor = max(0, remaining_ttl_ms − rtt)` with `rtt` that call's measured round-trip (0 when unknown), and the next beat fires `max(0, leadFloor − min(leadFloor/2, max(1s, 2×maxObservedRtt)))` after response receipt — recomputed from every response carrying the field. The lead-accounting skip check is bypassed (the schedule is exact), the first beat is derived from the create response instead of firing immediately (a lease capped to 1s gets its first beat at 500ms, inside the lease), and a transiently failed extend retries with the same idempotency key after `clamp(currentLead/4, 1s, 30s)`. Everything below is the **fallback** for servers that do not send the field; if the field disappears mid-flight, the fallback takes over seamlessly
